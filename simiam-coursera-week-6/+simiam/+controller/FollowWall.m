...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            if(strcmp(inputs.direction,'right'))
                ...
                if (ir_distances(4) > .2)
                    sorted_dist = sortrows(transpose([ir_distances(5),ir_distances(4),ir_distances(3)+.0001;5,4,3]))
                    if (sorted_dist(1,2) < sorted_dist(2,2))
                        p_2 = ir_distances_wf(:,sorted_dist(1,2));
                        p_1 = ir_distances_wf(:,sorted_dist(2,2));
                    else
                        p_1 = ir_distances_wf(:,sorted_dist(1,2));
                        p_2 = ir_distances_wf(:,sorted_dist(2,2));
                    end
               else
                    alpha = 1;
                    beta = 1;
                    d_fw = d_fw * 1.5;
                    fprintf('sensor four out\n');
                    sorted_dist = sortrows(transpose([ir_distances(5),ir_distances(4),ir_distances(3)+.0001;5,4,3]))
                    if (sorted_dist(1,2) < sorted_dist(2,2))
                        p_2 = ir_distances_wf(:,sorted_dist(1,2));
                        p_1 = ir_distances_wf(:,sorted_dist(2,2));
                ...
                fprintf('front_sensor = %0.4g\n', ir_distances(3));            
                if (ir_distances(2) > .2)
                    sorted_dist = sortrows(transpose([ir_distances(1),ir_distances(2),ir_distances(3);1,2,3]))
                    if (sorted_dist(1,2) < sorted_dist(2,2))
                        p_1 = ir_distances_wf(:,sorted_dist(1,2));
                        p_2 = ir_distances_wf(:,sorted_dist(2,2));
                    else
            ...
            u_fw_pp = u_fw_p - d_fw * (u_fw_p/norm(u_fw_p)); %[0;0];
             
            u_fw = alpha*u_fw_pp + beta*u_fw_tp;
            fprintf('(u_fw_pp(1),u_fw_pp(2)) = (%0.4g,%0.4g)\n', u_fw_pp(1),u_fw_pp(2));            
