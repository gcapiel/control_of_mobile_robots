...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            u_ao_gtg = blend_mix * [u_ao(1)/u_ao(2);1] + (1-blend_mix) * [u_gtg(1)/u_gtg(2);1];%zeros(2,1);
            ...
            u_straight = sqrt(u_gtg(1)^2+u_gtg(2)^2);
       
            time_to_goal = 2; % time from goal at which robot slows down
                
            % adjust v based on proxity to obstacle
            v_adjust = 1;
            if (u_straight / v) <= time_to_goal
                v_adjust = .5;
            end
            for i=1:5
                if ir_distances(i) < .2 % if any of the sensors detect a nearby obstable 
                    v_adjust = .75; % slow down
                end
            end
            v = v * v_adjust;
