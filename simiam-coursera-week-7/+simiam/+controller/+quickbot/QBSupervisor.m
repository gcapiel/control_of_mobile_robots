...
% see the LICENSE file included with this software
    ...
    %% METHODS
        ...
        %   See also controller/execute
            ...
            fprintf('entering state machine\n');
            ...
            if obj.check_event('unsafe')
                ...
                if obj.is_in_state('go_to_goal') && obj.check_event('at_obstacle')
                    if obj.sliding_left()
                        fprintf('sliding left\n');
                        obj.fw_direction = 'left';
                    end
                    if obj.sliding_right()
               ...
               if obj.is_in_state('follow_wall') && obj.progress_made()
                   fprintf('following wall and progress made\n');
                   change_state = 0;
                   fprintf('current follow direction: %s\n', obj.fw_direction);
                   if strcmpi(obj.fw_direction,'right') && ~obj.sliding_right()
                       change_state = 1;
                   end
                   if strcmpi(obj.fw_direction,'left') && ~obj.sliding_left()
                       change_state = 1;
                   end
                   if (change_state == 1)
               ...
               if condition_met == 0 && (obj.check_event('at_obstacle') || obj.is_in_state('avoid_obstacles'))
                   fprintf('no conditions and still at obstacle\n');
                   obj.switch_to_state('follow_wall');
               end
            end
            %% END CODE BLOCK %%
...
%             fprintf('current_pose: (%0.3f,%0.3f,%0.3f)\n', x, y, theta);
        ...
        function rc = progress_made(obj, state, robot)
            ...
            %% START CODE BLOCK %%
            d_g = norm([x-obj.goal(1);y-obj.goal(2)]);
            if (d_g < (obj.d_prog - .1))
                rc = true;
            end
